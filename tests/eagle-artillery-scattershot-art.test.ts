...
function shipsEffects(
  ...
  for (const name of names) {
    ...
    for (const row of rows[name]) {
      if (row.ParticleEmitter) expect(emitters[row.ParticleEmitter] ?? external.includes(row.ParticleEmitter), row.ParticleEmitter).toBeTruthy();
      if (row.Sound && sounds[row.Sound]) expect(sha256(`public/${sounds[row.Sound].path}`)).toBe(sounds[row.Sound].sha256);
    }
  }
}
...
describe('original Eagle Artillery art', () => {
  it('composes every level in dormant, activation, awake, attack, load, empty, upgrade and ruin states', () => {
    ...
    for (const level of EAGLE_ARTILLERY_ART_LEVELS) {
      ...
      const frames = [labels.idle, 25, 50, 75, labels.activating_end, labels.battleidle_start, labels.attack_start, labels.load_start, labels.empty];
      const looks = frames.map((frame) => drawable(EAGLE_ARTILLERY_GRAPH, eagleArtilleryPoses(level, 'active', frame)));
    ...
    for (const [kind, beam] of Object.entries(EAGLE_ARTILLERY_BEAMS))
      for (const frame of [beam.labels.warmup, beam.labels.loop, beam.labels.fadeend])
        drawable(EAGLE_ARTILLERY_GRAPH, eagleArtilleryBeamPoses(kind as keyof typeof EAGLE_ARTILLERY_BEAMS, frame));
    expect([EAGLE_ARTILLERY_EXPORTS.beamUp, EAGLE_ARTILLERY_EXPORTS.beamDown].every((e) => EAGLE_ARTILLERY_GRAPH.exports[e] !== undefined)).toBe(true);
  });
  it('schedules turret states from recorded activation, launches and emptiness', () => {
    ...
    const tower = (extra: Partial<EagleArtilleryTowerState>) =>
      ({ level: 2, ammunition: 30, wakeMs: 1125, stages: [], hitMs: 0, burstMs: 0, cooldownMs: 0, searchMs: 0, group: [], targetId: null, reticle: null, fired: 0, volleys: [], ...extra }) as EagleArtilleryTowerState;
    ...
    const firing = tower({ stages: [0, 0, 0, 0], awakeAt: 1, volleys: [{ index: 0, startedAt: 2, launches: [5, 5.7, 6.5] }] });
  ...
  it('ships the pinned battle effects, emitters and sounds', () => {
    shipsEffects(Object.values(EAGLE_ARTILLERY_EFFECTS), EAGLE_ARTILLERY_EFFECT_ROWS, EAGLE_ARTILLERY_EMITTERS, EAGLE_ARTILLERY_SOUNDS);
    for (const sound of Object.values(EAGLE_ARTILLERY_SOUNDS)) expect(sha256(`public/${sound.path}`)).toBe(sound.sha256);
  });
});
...
describe('original Scattershot art', () => {
  it('composes all 24 aimed throw bands with 15 frames, upgrade and ruin exports', () => {
    ...
    for (const level of SCATTERSHOT_ART_LEVELS) {
      ...
      expect(scatterCombat.directions[row.body as keyof typeof scatterCombat.directions]).toEqual({ views: 24, frames: 360, framesPerView: 15, fps: 30 });
      ...
      for (let view = 0; view < 24; view++)
        for (const frame of full ? [0, 5, 14] : [0]) {
          const look = drawable(SCATTERSHOT_GRAPH, scattershotPoses(level, 'active', view * 15 + frame));
    ...
    expect([scattershotView(1, 0), scattershotView(0, 1), scattershotView(-1, 0), scattershotView(0, -1)]).toEqual([0, 6, 12, 18]);
  });
  ...
  it('ships the pinned battle effects, emitters and sounds', () => {
    shipsEffects(Object.values(SCATTERSHOT_EFFECTS), SCATTERSHOT_EFFECT_ROWS, SCATTERSHOT_EMITTERS, SCATTERSHOT_SOUNDS, Object.keys(scatterCombat.externalParticles));
    for (const sound of Object.values(SCATTERSHOT_SOUNDS)) expect(sha256(`public/${sound.path}`)).toBe(sound.sha256);
  });
});
