...
describe.each(['cannon', 'archertower'] as const)('%s construction progression', (kind) => {
  ...
  it('enforces each Town Hall count and preserves grandfathered pieces', () => {
    ...
    const extra = makeBuilding(m.state.nextId++, kind, 2, 2, 12);
    m.state.obstacles = [];
  ...
  it('normalizes old health without restarting a pending timer or changing its paid cost', () => {
    ...
    const b = save.buildings.find((b) => b.kind === kind)!;
    b.level = 2;
    b.maxHp = 2000;
    b.hp = 1000;
