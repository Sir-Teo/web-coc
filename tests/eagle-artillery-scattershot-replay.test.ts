...
function observe(battle: Battle, seen: Observed) {
  ...
  if (ea) {
    seen.awake = Math.max(seen.awake, Object.values(ea.towers).filter((t) => t.awakeAt !== undefined).length);
    seen.shells = Math.max(seen.shells, Object.values(ea.towers).reduce((n, t) => n + t.fired, 0));
  }
  if (battle.units.some((u) => u.late?.eagleArtillery?.push)) seen.pushes++;
  if (ss) {
    seen.throws = Math.max(seen.throws, Object.values(ss.towers).reduce((n, t) => n + t.fired, 0));
...
describe('Eagle Artillery and Scattershot campaign gates', () => {
  it('unlocks every village whose remaining late families are these two', () => {
    expect(lateCampaignIssues([[1000031, 0, 0, 7], [1000067, 0, 0, 7]])).toEqual([]);
    const blocked = (index: number) => nativeCampaignIssues(index);
    for (const index of [61, 63, 65, 68, 70]) expect(blocked(index), NATIVE_CAMPAIGN[index].name).toEqual([]);
    for (const index of [67, 69, 71, 72, 73, 74, 75, 76, 77, 78, 79, 80, 81, 82, 83, 84, 85, 86, 88, 89])
      expect(blocked(index)).not.toEqual(expect.arrayContaining([expect.stringMatching(/Eagle Artillery|Scattershot/)]));
  ...
  it('accepts these buildings only in version 44 campaign recordings', () => {
    const data = (version: number, practice: boolean, kind: 'eagleartillery' | 'scattershot') => {
      const setup = lateSetup(65, { giant: 1 }, {}, [makeBuilding(1, kind, 20, 20, 7), makeBuilding(2, 'townhall', 2, 2, 1)]);
      const replay = replayData(setup, [], 0);
      replay.version = version;
      if (practice) {
        replay.initial = { ...replay.initial, practice: true, catalog: undefined, scenery: undefined, availableLoot: undefined, lootRoom: undefined };
    ...
    expect(parseReplayFile(JSON.stringify(makeReplayFile(large))).initial.buildings).toHaveLength(820);
    const legacy = structuredClone(large);
    legacy.version = 43;
    legacy.initial.buildings = Array.from({ length: 601 }, (_, i) => makeBuilding(i + 1, 'wall', 2 + (i % 44), 2 + Math.floor(i / 44), 1));
...
describe('Eagle Artillery and Scattershot replays', () => {
  it(
    'records Where Eagles Dare from the campaign and reconstructs it across backward seeks',
    () => {
      ...
      m.state.troopLevels = Object.fromEntries(TROOP_KEYS.map((k) => [k, maxTroopLevel(k)])) as typeof m.state.army;
      m.startCampaign(65);
      const b = m.battle!;
      ...
      // Every tower fires at least two full volleys.
      expect(seen.shells).toBeGreaterThanOrEqual(48);
  ...
  for (const [index, army, spells, plan, wanted] of [
    ...
    [
      ...
      [
        ...
        ...[0, 1, 2, 3].map((i) => ({ step: 100, type: 'spell' as const, kind: 'lightning' as const, x: 20 + i * 2, y: 24 })),
      ],
    ...
    it(
      `reconstructs ${NATIVE_CAMPAIGN[index].name} (${index}) live combat across backward seeks`,
      () => {
        const setup = lateSetup(index, army, spells);
        setup.troopLevels = Object.fromEntries(TROOP_KEYS.map((k) => [k, maxTroopLevel(k)])) as typeof setup.troopLevels;
        ...
        for (let step = 0; step < steps; step++) {
          ...
          expect(model.battle!.units.length - placed).toBe(deployments.filter((d) => d.step === step && d.type === 'troop').length);
          if ([1, 121, 261, 600, 1000].includes(step)) snapshots.set(step, JSON.stringify(model.battle));
  ...
  it('uses the ungated native layouts only for villages still waiting on other families', () => {
    ...
    expect(nativeLayout(80).filter((b) => b.kind === 'eagleartillery').map((b) => b.level)).toEqual([5, 5]);
    expect(nativeLayout(80).filter((b) => b.kind === 'scattershot').map((b) => b.level)).toEqual([3, 3, 3, 3]);
