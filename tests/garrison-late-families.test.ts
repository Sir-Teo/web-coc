...
import { stepAttackerVsDefenders, stepDefenders, type GarrisonDefender } from '../src/game/defenders';
...
it('resolves every later family at its pinned source row and splits the attack timer', () => {
  const table = [
    ['electrodragon', 'Electro Dragon', { hp: 4400, damage: 1120, rate: 3.5, range: 2.5, speed: 1.6, flying: true, airTargets: true, groundTargets: true, firstAttackDelay: 2.875, recovery: 0.625 }],
    ['golem', 'Golem', { hp: 7500, damage: 168, rate: 2.4, range: 1, speed: 1.5, deathDamage: 700, deathRadius: 1.5, firstAttackDelay: 1.5, recovery: 0.9 }],
    ['golemite', 'Golemite', { hp: 1440, damage: 42, rate: 3, range: 0.5, deathDamage: 140, deathRadius: 1.2, firstAttackDelay: 1.5, recovery: 1.5 }],
    ['witch', 'Witch', { hp: 470, damage: 115.5, rate: 0.7, range: 4, splash: 0.3, projectile: 'Witch_projectile', projectileSpeed: 5, projectileFixed: true, firstAttackDelay: 0.7, recovery: 0 }],
    ['summonedskeleton', 'Skeleton', { hp: 30, damage: 25, rate: 1, range: 0.4, speed: 3, spawnIdle: 0.5, airTargets: false }],
    ['bowler', 'Bowler', { hp: 470, damage: 211.2, rate: 2.2, range: 3, splash: 0.3, projectile: 'trollBoulder_lvl3', projectileSpeed: 4.75, airTargets: false, firstAttackDelay: 0.4, recovery: 0.7 }],
    ['lavahound', 'Defensive Lava Hound', { hp: 8000, damage: 40, rate: 2, range: 0.25, speed: 2.5, flying: true, airTargets: false, groundTargets: true, deathDamage: 350, deathRadius: 1.2, projectile: 'hound_projectile', projectileSpeed: 2, firstAttackDelay: 1.5, recovery: 0.5 }],
    ['lavapup', 'Lava Pup', { hp: 50, damage: 35, rate: 1, range: 2.25, speed: 4, flying: true, airTargets: true, groundTargets: true, projectile: 'tinyhound_projectile', projectileSpeed: 3, pushbackSpeed: 4 }],
    ['electrotitan', 'Electro Titan', { hp: 7700, damage: 300, rate: 1.5, range: 1.25, speed: 2, airTargets: true, groundTargets: true, firstAttackDelay: 1.5, recovery: 0 }],
    ['goldendragon', 'Golden Dragon', { hp: 32000, housing: 100, damage: 1750, rate: 1.25, range: 2.5, splash: 1, flying: true, firstAttackDelay: 1.25 }],
    ['momma', 'MOMMA', { hp: 50000, housing: 100, damage: 10000, rate: 5, range: 1.25, splash: 3, flying: false, airTargets: true, groundTargets: true, firstAttackDelay: 1.5, recovery: 3.5 }],
    ['royalghost', 'Royal Ghost', { hp: 300, damage: 720, rate: 1, range: 0.5, speed: 2, airTargets: false, firstAttackDelay: 1, recovery: 0, frost: { duration: 4, scale: 0.5 } }],
  ] as const;
  ...
  expect(garrisonStats('electrodragon', 3).chain).toEqual({ distance: 3, depth: 5, factor: 1, delay: 0.128, reduction: 0.2 });
  expect(garrisonStats('golem', 8).secondary).toEqual({ character: 'Golemite', count: 3, distance: 1.5, offset: 0, randomize: false });
  // The pinned defensive row releases 13 pups, not the offensive row's 18.
  expect(garrisonStats('lavahound', 6).secondary).toEqual({ character: 'Lava Pup', count: 13, distance: 3.5, offset: 1, randomize: true });
  expect(characterLevel('Lava Hound', 6)).toMatchObject({ SecondaryTroopCnt: '18' });
  expect(garrisonStats('witch', 4).summon).toEqual({ character: 'Skeleton', count: 4, limit: 12, level: 1, time: 0.875, cooldown: 7, distance: 1.5 });
  ...
  expect(garrisonDeathSpell(garrisonStats('electrodragon', 3))).toEqual({
    spell: 'ElectroDragonDie', level: 6, damage: 85, radius: 2, hits: 6, firstHit: 1.9, interval: 0.4,
    randomRadius: 2.5, randomOnlyGfx: false, heroDamageScale: 1,
  });
  expect(garrisonAura(garrisonStats('electrotitan', 2))).toEqual({
    spell: 'Electro Titan Aura', level: 2, damage: 40, radius: 3.5, hits: 4000, firstHit: 0.4, interval: 0.4,
    randomRadius: 0, randomOnlyGfx: false, heroDamageScale: 0.25,
  });
  expect(garrisonStealth(garrisonStats('royalghost', 7))).toEqual({ duration: 10, ignoreObstacles: true });
  // Healing rows use the 200 ms windup threshold (Defending Builder: DPS -50 at 750 ms).
  expect(splitTiming(characterLevel('Defending Builder', 1)!)).toEqual({ period: 0.75, recovery: 0.55, windup: 0.2, charge: 0, firstHit: 0.2 });
  // The earlier ten families keep their frozen interpretation and gain no later-family fields.
  for (const [kind, level] of [['dragon', 7], ['balloon', 8], ['goblin', 7], ['archer', 9], ['dragon', 5], ['pekka', 8], ['valkyrie', 7], ['headhunter', 3], ['superminion', 9], ['babydragon', 6]] as const)
    expect(garrisonStats(kind, level), kind).toMatchObject({ split: false, recovery: 0, chain: undefined, secondary: undefined, summon: undefined, bounce: undefined, aura: undefined, frost: undefined, projectileFixed: false });
...
it('reproduces the source integer sine table, LogicRandom and spawn angles', () => {
  expect([0, 30, 59, 90, 149, 180, 270, 299, 360, -90].map(sourceSin)).toEqual([0, 512, 878, 1024, 527, 0, -1024, -896, 0, -1024]);
...
it('Electro Dragons chain lightning through five attackers with delayed, reduced jumps', () => {
  ...
  expect(jumps.map((j) => j.at)).toEqual([2.9 + 0.128, 2.9 + 0.256, 2.9 + 0.384, 2.9 + 0.512].map((t) => expect.closeTo(t, 9)));
  expect(battle.units.map((u) => 50000 - u.hp)).toEqual([1120, 896, 716.8, 573.44, 458.752, 0, 0].map((d) => expect.closeTo(d, 6)));
...
it('Electro Dragon death casts six deterministic ElectroDragonDie bolts on both layers', () => {
  ...
  battle.units = [unit(1, 'giant', 20, 20), unit(2, 'dragon', 21, 20), unit(3, 'swordsman', 20, 21, { hero: 'king' }), unit(4, 'giant', 30, 30)];
  dragon.stunnedUntil = 999;
  advance(battle, 1);
  kill(battle, dragon);
  step(battle);
  expect(dragon.bolts!.map((b) => b.at)).toEqual([0, 1, 2, 3, 4, 5].map((k) => expect.closeTo(1 + 1.9 + 0.4 * k, 9)));
  expect(dragon.bolts![0]).toMatchObject({ x: 20, y: 20 });
  advance(battle, 4.2);
  expect(dragon.bolts!.every((b) => b.done)).toBe(true);
  const expected = battle.units.map((u) =>
...
it('Golems split into three pushed Golemites after their ground death damage', () => {
  ...
  battle.units = [unit(1, 'giant', 21.5, 20.5), unit(2, 'dragon', 20.5, 21), unit(3, 'giant', 23, 20.5)];
  ...
  const vector = (angle: number) => [Math.trunc((768 * sourceCos(angle)) / 1024) / 512, Math.trunc((768 * sourceSin(angle)) / 1024) / 512];
  expect(golemites.map((d) => [d.push!.toX - 20.5, d.push!.toY - 20.5])).toEqual([59, 179, 299].map(vector));
  ...
  for (const d of golemites) {
    ...
    expect([d.x, d.y]).toEqual([d.push!.fromX + (d.push!.toX - d.push!.fromX) * f, d.push!.fromY + (d.push!.toY - d.push!.fromY) * f]);
  }
  ...
  step(battle);
  const near = battle.units.map((u, i) => (Math.hypot(u.x - golemites[0].x, u.y - golemites[0].y) <= 1.2 && u.kind !== 'dragon' ? before[i] - 140 : u.hp));
...
it('Witches summon Skeleton waves on the source cycle, capped by SummonLimit', () => {
  ...
  const wave = () => battle.defenders!.filter((d) => d.kind === 'summonedskeleton') as GarrisonDefender[];
  ...
  const retaliate = () => stepAttackerVsDefenders(battle, archer, { damage: 1, speed: 3, range: 3.5, rate: 1 }, 0.05, [], () => {}, () => {});
  ...
  kill(second, other);
  step(second);
  expect(second.defenders!.filter((d) => d.kind === 'summonedskeleton').every((d) => d.hp === 0)).toBe(true);
});
...
it('Bowler boulders strike the target, then bounce four tiles further along the throw', () => {
  ...
  battle.units = [unit(1, 'giant', 13, 10), unit(2, 'giant', 13.25, 10), unit(3, 'giant', 17, 10), unit(4, 'dragon', 17, 10)];
  ...
  advance(battle, 0.7);
  expect(battle.units.map((u) => 50000 - u.hp)).toEqual([211.2, 211.2, 0, 0].map((d) => expect.closeTo(d, 9)));
  ...
  expect(battle.units.map((u) => 50000 - u.hp)).toEqual([211.2, 211.2, 211.2, 0].map((d) => expect.closeTo(d, 9)));
...
it('Lava Hounds attack ground only and burst into thirteen randomized Lava Pups', () => {
  ...
  pups.forEach((pup, i) => {
    ...
    expect(pup.push!.fromX - hound.x).toBeCloseTo(Math.trunc((512 * sourceCos(angle)) / 1024) / 512, 9);
    const units = random(1792) + 896;
    expect(pup.push!.toX - pup.push!.fromX).toBeCloseTo(Math.trunc((units * sourceCos(angle)) / 1024) / 512, 9);
...
it('Electro Titans pulse their aura every 400 ms on both layers, even while stunned', () => {
  ...
  battle.units = [unit(1, 'giant', 12, 10), unit(2, 'dragon', 10, 13.4), unit(3, 'giant', 14, 10), unit(4, 'swordsman', 10, 8, { hero: 'king' })];
  advance(battle, 2);
...
it('Golden Dragons and M.O.M.M.A splash only the struck layer', () => {
  ...
  battle.units = [unit(1, 'giant', 12, 10), unit(2, 'giant', 12.9, 10), unit(3, 'dragon', 12, 10.5), unit(4, 'giant', 13.2, 10)];
  ...
  second.units = [unit(1, 'giant', 11, 10), unit(2, 'giant', 13.5, 10), unit(3, 'giant', 14.5, 10), unit(4, 'dragon', 11, 10.2)];
...
it('keeps later-family combat, spawns and summons deterministic through JSON restoration', () => {
  ...
  for (const [index, [kind, level]] of LATE.entries())
    spawnGarrisonDefender(battle, kind, level, 1, 6 + (index % 6) * 2, 6 + Math.floor(index / 6) * 3, 0);
  battle.units = [
  ...
  advance(battle, 4);
  for (const defender of battle.defenders!) if (defender.kind === 'golem' || defender.kind === 'lavahound' || defender.kind === 'electrodragon') kill(battle, defender as GarrisonDefender);
  advance(battle, 0.5);
...
it('keeps the bunkerless Besieged roster recorded and verifiably inert', () => {
  ...
  expect(resolvedCampaignGarrison(73)).toEqual({ castle: null, troops: [{ kind: 'electrodragon', level: 3, count: 3 }] });
  ...
  for (const index of [56, 67, 69, 72, 74, 76, 77, 83, 89]) expect(inertCampaignGarrison(index)).toBeUndefined();
});
