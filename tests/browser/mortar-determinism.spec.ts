...
const cases = [1, 6, 8, 11, 16, 17, 18];

for (const fixture of cases)
  test(`complete combat state matches Node at every step: Mortar ${fixture}`, async ({
    page,
    browserName,
  }) => {
    const { mortarBattle } = await modules.ssrLoadModule('/tests/fixtures/mortar-battle.ts');
    const m = mortarBattle(fixture);
    ...
    await fs.writeFile(
      `output/playtest/mortar-combat-determinism-${label}-${browserName}.json`,
      JSON.stringify(
