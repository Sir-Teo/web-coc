...
for (const witness of witnesses)
  test(`native Mortar ${witness.category} matches original pixels and survives context restoration`, async ({
    ...
    const report = await page.evaluate(async (reference) => {
      ...
      const { default: world } = await import('/reference/mortar/runtime.json');
      ...
      const views = reference.cases.map((c) => {
        const view = new NativeSceneView(scene, 'mortar-test-world');
    ...
    await (
      ...
      `output/playtest/native-mortar-${witness.category}-${browserName}.json`,
      JSON.stringify(report, null, 2),
    );
    console.log('Native Mortar GPU comparison', browserName, JSON.stringify(report));
