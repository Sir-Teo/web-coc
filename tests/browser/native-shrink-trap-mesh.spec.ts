...
test('native Shrink Trap clips and original particles match source pixels and survive context restoration', async ({
  ...
  const report = await page.evaluate(async (reference) => {
    ...
    preloadNativeMeshes(scene, world, 'shrink-trap-test-world');
    scene.load.image(
      'shrink-trap-source-reference',
      '/tests/fixtures/native-shrink-trap-mesh/reference.png',
    );
    ...
    const views = reference.cases.map((c) => {
      const view = new NativeSceneView(scene, `shrink-trap-test-${c.graph}`);
    ...
    const image = scene.add.image(0, 0, 'shrink-trap-source-reference').setOrigin(0, 0);
    ...
    return {
      ...
      tintedTextures: scene.textures
        .getTextureKeys()
        .filter((k) => k.startsWith('shrink-trap-test') && k.includes(':color:')).length,
      glError: gl.getError(),
      glErrors,
      clip: {
  ...
  await (
    ...
    `output/playtest/native-shrink-trap-${browserName}.json`,
    JSON.stringify(report, null, 2),
  );
  console.log('Native Shrink Trap GPU comparison', browserName, JSON.stringify(report));
  await page.screenshot({
    path: `output/playtest/native-shrink-trap-${browserName}.png`,
    clip: report.clip,
  });
