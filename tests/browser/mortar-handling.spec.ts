...
const audit = (page: Page) => page.evaluate(() => window.__mortarHandlingAudit);
...
async function setup(page: Page) {
  ...
  await page.evaluate(async () => {
    ...
    audio.unlock();
    const record = (window.__mortarHandlingAudit = { events: [], samples: [], tones: [] });
    m.onEffect = (fx) => {
      if (fx.type.startsWith('mortar-')) record.events.push(structuredClone(fx));
      scene.effect(fx);
    };
  ...
  await expect
    .poll(() =>
      page.evaluate(() =>
        ['mortar-mortar_pickup_01', 'mortar-mortar_place_02'].every((key) =>
...
for (const width of [1440, 390])
  test(`native Mortar move, blocked drop and placement use one original sound at ${width}px`, async ({
    page,
    browserName,
  }) => {
    ...
    expect((await audit(page)).events.map((e) => e.type)).toEqual(['mortar-pickup']);
    ...
    expect(state.events).toEqual([
      { type: 'mortar-pickup', sourceId: 3, x: 11.5, y: 11.5 },
      { type: 'mortar-place', sourceId: 3, x: 7.5, y: 11.5 },
    ]);
    expect(state.samples.map((s) => s.sample)).toEqual([
      'mortar-mortar_pickup_01',
      'mortar-mortar_place_02',
    ]);
    ...
    const grass = await page.evaluate(() => {
      const { scene } = window.__game;
      scene.paused = true;
      const event = scene.mortarPresentation.homeEffects.at(-1);
      scene.renderClock = (event.at + 0.14) * 1000;
      scene.drawOverlay();
      return [...scene.mortarPresentation.effects.values()].flatMap((v) =>
        v.objects.map((o) => ({
          x: o.x,
          y: o.y,
          depth: o.depth,
          kind: o.getData('nativeMortarEffect').key.includes(':home-place:') ? 'place' : 'pickup',
        })),
      );
    });
    ...
    await page.screenshot({
      path: `output/playtest/mortar-place-${width}-${browserName}.png`,
    });
    await page.evaluate(() => {
      const { scene } = window.__game;
    ...
    expect(await page.evaluate(() => window.__game.scene.mortarPresentation.effects.size)).toBe(0);
  });

test('home Mortar effects respect reduced motion, mute, cancellation and battle transitions', async ({
  page,
}) => {
  await setup(page);
  const result = await page.evaluate(() => {
    ...
    draw();
    const reduced = {
      grass: scene.mortarPresentation.effects.size,
    ...
    draw();
    const cancelled = {
      events: scene.mortarPresentation.homeEffects.length,
      sounds: audio.samples.active.size,
    };
    ...
    draw();
    const active = scene.mortarPresentation.effects.size;
    scene.sys.pause();
    ...
    draw();
    const battle = {
      home: scene.mortarPresentation.homeEffects.length,
      grass: scene.mortarPresentation.effects.size,
      sounds: audio.samples.active.size,
    };
    m.finishBattle();
