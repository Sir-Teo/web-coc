  ...
  test(`native ${witness.category} matches original pixels and survives context restoration`, async ({
    ...
    const report = await page.evaluate(async (reference) => {
      ...
      for (const family of new Set(reference.cases.map((c) => c.family)))
        preloadNativeMeshes(scene, worlds[family], `archer-test-${family}`);
      scene.load.image(
        'archer-source-reference',
      ...
      const views = reference.cases.map((c) => {
        ...
        const view = new NativeSceneView(scene, `archer-test-${c.family}`);
      ...
      const image = scene.add
        .image(0, -reference.sourceY, 'archer-source-reference')
        .setOrigin(0, 0);
      ...
      return {
        ...
        tintedTextures: scene.textures
          .getTextureKeys()
          .filter((k) => k.startsWith('archer-test') && k.includes(':color:')).length,
        glError: gl.getError(),
        glErrors,
        clip: {
    ...
    console.log('Native Archer Tower GPU comparison', browserName, witness.category, {
