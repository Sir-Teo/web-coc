...
import witness from '../fixtures/native-bomb-tower-mesh/manifest.json' with { type: 'json' };

test.use({ viewport: { width: 1250, height: 1650 } });

test('native Bomb Tower bodies, defenders and bombs match source pixels and survive context restoration', async ({
  page,
  browserName,
}) => {
  ...
  const report = await page.evaluate(async (reference) => {
    ...
    scene.load.image(
      ...
      '/tests/fixtures/native-bomb-tower-mesh/reference.png',
    );
  ...
  console.log('Native Bomb Tower GPU comparison', browserName, JSON.stringify(report));
  await page.screenshot({
    path: `output/playtest/native-bomb-tower-mesh-${browserName}.png`,
    clip: report.clip,
  });
