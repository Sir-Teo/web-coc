...
for (const fixture of cases)
  test(`complete combat state matches Node at every step: Archer Tower ${fixture}`, async ({
    ...
    const { archerTowerBattle } = await modules.ssrLoadModule(
      '/tests/fixtures/archer-tower-battle.ts',
    );
    const m = archerTowerBattle(fixture);
    ...
    await fs.writeFile(
      `output/playtest/archer-tower-combat-determinism-${label}-${browserName}.json`,
      JSON.stringify(
