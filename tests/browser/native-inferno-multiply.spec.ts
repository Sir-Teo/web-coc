...
import index from '../fixtures/native-inferno-multiply/index.json' with { type: 'json' };
...
const witnesses = [...index.map((v) => ({ ...v, folder: 'native-inferno-multiply' }))].flatMap(
  ({ category, folder }) => {
...
for (const witness of witnesses)
  test(`native ${witness.category} matches original pixels and survives context restoration`, async ({
    ...
    const report = await page.evaluate(async (reference) => {
      ...
      const { default: inferno } = await import('/reference/inferno/multiply-runtime.json');
      ...
      return {
        ...
        tintedTextures: scene.textures
          .getTextureKeys()
          .filter((k) => k.startsWith('garrison-test') && k.includes(':color:')).length,
        glError: gl.getError(),
        glErrors,
        clip: {
    ...
    console.log('Native Inferno GPU comparison', browserName, JSON.stringify(report));
