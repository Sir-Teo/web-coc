...
for (const viewport of [
  ...
  test(`finite loot, unlimited scouting, overflow and replay at ${viewport.width}px`, async ({
    ...
    const trophies = await page.evaluate(async () => {
      const { freshCampaignLoot } = await import('/src/game/campaign-loot.ts');
      const m = window.__game.model;
      m.state.campaignLoot = freshCampaignLoot();
      m.state.campaignLoot.remaining[0] = { gold: 700, elixir: 300 };
      m.state.campaignLoot.remaining[1] = { gold: 0, elixir: 0 };
      m.state.stars[0] = 1;
    ...
    await expect(page.locator('.campaign-loot').first()).toContainText('700');
    ...
    await expect(page.locator('[data-loot="gold"]')).toHaveText('700');
    ...
    await page.locator('[data-action="spell:lightning"]').click();
    const point = await page.evaluate(() => {
      const { scene, model } = window.__game;
      const b = model.battle.buildings.find((b) => b.kind === 'goldmine');
      ...
      return scene.screenFor(b.x + 1.5, b.y + 1.5);
    });
    await page.mouse.click(point.x, point.y);
    await expect
      .poll(() => page.evaluate(() => window.__game.model.battle.spells.lightning))
      .toBe(0);
    await expect
      .poll(() => page.evaluate(() => window.__game.model.battle.lootTaken.gold))
      .toBeGreaterThan(0);
    await page.evaluate(() => window.advanceTime(320000));
    ...
    const saved = await page.evaluate(() => {
      const m = window.__game.model;
      return {
        inventory: m.state.campaignLoot,
        result: m.battle.result,
        trophies: m.state.trophies,
      };
    });
    expect(saved.trophies).toBe(trophies);
    expect(saved.inventory.remaining[0].gold).toBeLessThan(700);
    ...
    await page.waitForFunction(() => window.__game?.scene.ready);
    await page.locator('#loading').waitFor({ state: 'detached' });
    expect(await page.evaluate(() => window.__game.model.state.campaignLoot)).toEqual(
      saved.inventory,
    );
