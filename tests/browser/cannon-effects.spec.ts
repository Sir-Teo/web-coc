...
function snapshot() {
  ...
  const view = (v) =>
    v.objects.map((o) => ({
      ...
      body: o.getData('nativeCannon'),
      effect: o.getData('nativeCannonEffect'),
    }));
  return {
    elapsed: m.battle.elapsed,
    history: structuredClone(m.battle.cannons),
    physical: structuredClone(m.battle.projectiles),
    projectiles: [...scene.cannonPresentation.projectiles].map(([key, v]) => [key, view(v)]),
    towers: [...scene.cannonPresentation.towers].map(([id, v]) => [id, view(v)]),
    effects: [...scene.cannonPresentation.effects]
...
for (const [width, height] of [
  ...
  test(`original Cannon aiming, firing, destruction and audio reconstruct at ${width}×${height}`, async ({
    page,
    browserName,
  }) => {
    ...
    await page.evaluate(async () => {
      ...
      scene.sync();
      scene.drawOverlay();
      window.__cannonAudit = { baseline: scene.game.renderer.listenerCount('losewebgl') };
    ...
    await page.waitForFunction(
      () =>
        [...window.__game.audio.samples.buffers.keys()].filter((k) => k.startsWith('cannon-'))
          .length === 5,
    );
    ...
    const fire = phases.get(0.3)!;
    expect(fire.history[6].shots).toHaveLength(1);
    expect(fire.physical).toHaveLength(1);
    ...
    await page.evaluate(() => {
      ...
      if (!m.battle.finished) throw Error('Cannon fixture did not finish');
      window.__cannonAudit.final = structuredClone(m.battle);
      m.returnHome();
      window.__cannonAudit.home = structuredClone(m.state);
    ...
    for (const at of [6, 0.05, 0.75, 0.3, 2, 0.6, 0.65, 1, 0.3])
      expect(await seek(at)).toEqual(phases.get(at));
    await page.evaluate(
      () => new Promise((resolve) => requestAnimationFrame(() => requestAnimationFrame(resolve))),
    );
    await page.screenshot({
      path: `output/playtest/cannon-fire-${width}-${browserName}.png`,
      animations: 'disabled',
    });
    const audio = await page.evaluate(() => {
      ...
      draw();
      const reduced = {
        effects: scene.cannonPresentation.effects.size,
        projectiles: scene.cannonPresentation.projectiles.size,
      };
      m.state.settings.reducedMotion = false;
      return { playing, paused, resumed, muted, reduced };
    });
    expect(audio.playing).toHaveLength(1);
    expect(audio.playing[0].sample).toBe('cannon-cannon_08');
    expect(audio.playing[0].gain).toBeCloseTo(0.096, 8);
    expect(audio.playing[0].rate).toBe(2);
    expect(audio.paused).toEqual([]);
    expect(audio.muted).toEqual([]);
    expect(audio.resumed).toEqual(audio.playing);
    expect(audio.reduced).toEqual({ effects: 0, projectiles: 0 });
    const death = await page.evaluate(() => window.__cannonAudit.final.cannons[6].destroyedAt);
    ...
    await page.screenshot({
      path: `output/playtest/cannon-destruction-${width}-${browserName}.png`,
      animations: 'disabled',
    });
    ...
    const cleanup = await page.evaluate(() => {
      ...
      const same =
        JSON.stringify(m.battle.result) === JSON.stringify(window.__cannonAudit.final.result);
      m.returnHome();
      scene.sync();
      scene.drawOverlay();
      return {
    ...
    await (
      ...
      `output/playtest/cannon-live-${width}-${browserName}.json`,
      JSON.stringify(
        {
          phases: [...phases],
          audio,
...
test('a frozen original shot, trail and smoke survive WebGL context restoration pixel for pixel', async ({
  page,
  browserName,
}) => {
  await page.goto('/');
  ...
  const report = await page.evaluate(async () => {
    ...
    scene.cannonPresentation.clear();
    scene.cameras.main.setZoom(2).centerOn(860, 650).setBackgroundColor('#304135');
    const draw = () =>
      scene.cannonPresentation.render(m.battle.buildings, m.battle, m.battle.elapsed, false, iso);
    draw();
  ...
  await page.screenshot({ path: `output/playtest/cannon-context-restored-${browserName}.png` });
  await (
    await import('node:fs/promises')
  ).writeFile(
    `output/playtest/cannon-context-restored-${browserName}.json`,
