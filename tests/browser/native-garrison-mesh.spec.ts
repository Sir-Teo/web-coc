...
for (const witness of witnesses)
  test(`native garrison ${witness.category} matches original pixels and survives context restoration`, async ({
    page,
    browserName,
  }) => {
    ...
    const report = await page.evaluate(async (reference) => {
      ...
      const views = reference.cases.map((c) => {
        ...
        const view = new NativeSceneView(scene, `garrison-test-${c.family}`);
      ...
      const image = scene.add
        .image(0, -reference.sourceY, 'garrison-source-reference')
        .setOrigin(0, 0);
    ...
    await (
      ...
      `output/playtest/native-garrison-${witness.category}-${browserName}.json`,
      JSON.stringify(report, null, 2),
    );
    console.log('Native garrison GPU comparison', browserName, JSON.stringify(report));
    await page.screenshot({
      path: `output/playtest/native-garrison-${witness.category}-${browserName}.png`,
      clip: report.clip,
    });
    ...
    if (witness.cases.some((c) => c.family === 'dragon7'))
      expect(report.blended).toBeGreaterThan(0);
