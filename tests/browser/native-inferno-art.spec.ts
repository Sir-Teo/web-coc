...
import index from '../fixtures/native-inferno-art/index.json' with { type: 'json' };
...
const witnesses = [...index.map((v) => ({ ...v, folder: 'native-inferno-art' }))].flatMap(
  ({ category, folder }) => {
...
for (const witness of witnesses)
  test(`native ${witness.category} matches original pixels and survives context restoration`, async ({
    page,
    browserName,
  }) => {
    ...
    const report = await page.evaluate(async (reference) => {
      ...
      const { default: inferno } = await import('/reference/inferno/art-runtime.json');
      for (const id of Object.keys(inferno.clips))
        inferno.exports[`witness_clip_${id}`] = Number(id);
      const worlds = { inferno };
      ...
      for (const family of new Set(reference.cases.map((c) => c.family)))
        preloadNativeMeshes(scene, worlds[family], `inferno-test-${family}`);
      scene.load.image(
        'inferno-source-reference',
      ...
      const views = reference.cases.map((c) => {
        const world = worlds[c.family];
        const view = new NativeSceneView(scene, `inferno-test-${c.family}`);
      ...
      const image = scene.add
        .image(0, -reference.sourceY, 'inferno-source-reference')
        .setOrigin(0, 0);
      ...
      return {
        ...
        tintedTextures: scene.textures
          .getTextureKeys()
          .filter((k) => k.startsWith('inferno-test') && k.includes(':color:')).length,
        glError: gl.getError(),
        glErrors,
        clip: {
    ...
    await (
      ...
      `output/playtest/native-${witness.category}-${browserName}.json`,
      JSON.stringify(report, null, 2),
    );
    console.log('Native Inferno GPU comparison', browserName, witness.category, {
      cases: report.cases.length,
      maximumMeanError: Math.max(...report.cases.map((c) => c.meanError)),
      maximumChannelError: Math.max(...report.cases.map((c) => c.maximum)),
      singleTextureChanges: report.singleTextureChanges,
      contextChanges: report.contextChanges,
    });
    await page.screenshot({
      path: `output/playtest/native-${witness.category}-${browserName}.png`,
      clip: report.clip,
    });
    ...
    if (witness.cases.some((c) => c.export.startsWith('dark_tower_lvl1')))
