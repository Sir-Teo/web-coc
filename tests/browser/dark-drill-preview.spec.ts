...
test('original Drill placement preview tracks tiers, validity and cancellation', async ({
  page,
  browserName,
}) => {
  await page.goto('/');
  ...
  const result = await page.evaluate(async () => {
    ...
    model.state.buildings = [makeBuilding(999, 'darkdrill', 20, 20, 1)];
    model.move(999);
    const point = (x, y) => {
    ...
    for (let level = 1; level <= 11; level++) {
      ...
      model.state.buildings[0].stored = 100000;
      scene.sync();
      ...
      const view = scene.darkDrillPresentation.ghost;
      if (!view?.objects.length) throw Error('Missing native preview');
    ...
    model.cancel();
    scene.sync();
    const cancelled = !scene.darkDrillPresentation.ghost;
    model.placement = 'darkdrill';
    model.moving = null;
    scene.sync();
    scene.updateGhost(point(25, 25));
    const fresh = scene.darkDrillPresentation.ghost.objects.length;
    model.placement = 'goldmine';
    scene.sync();
    const switched = !scene.darkDrillPresentation.ghost && scene.ghost.alpha === 0.72;
    model.cancel();
    model.move(999);
    scene.sync();
    scene.updateGhost(point(25, 25));
