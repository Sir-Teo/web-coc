...
import index from '../fixtures/native-dark-drill-art/index.json' with { type: 'json' };
...
const witnesses = [...index.map((v) => ({ ...v, folder: 'native-dark-drill-art' }))].flatMap(
  ({ category, folder }) => {
...
for (const witness of witnesses)
  test(`native ${witness.category} matches original pixels and survives context restoration`, async ({
    ...
    const report = await page.evaluate(async (reference) => {
      ...
      const { default: drill } = await import('/reference/dark-drill/art-runtime.json');
      for (const id of Object.keys(drill.clips)) drill.exports[`witness_clip_${id}`] = Number(id);
      const worlds = { 'dark-drill': drill };
      ...
      for (const family of new Set(reference.cases.map((c) => c.family)))
        preloadNativeMeshes(scene, worlds[family], `drill-test-${family}`);
      scene.load.image(
        'drill-source-reference',
      ...
      const views = reference.cases.map((c) => {
        ...
        const view = new NativeSceneView(scene, `drill-test-${c.family}`);
      ...
      const image = scene.add
        .image(0, -reference.sourceY, 'drill-source-reference')
        .setOrigin(0, 0);
      ...
      return {
        ...
        tintedTextures: scene.textures
          .getTextureKeys()
          .filter((k) => k.startsWith('drill-test') && k.includes(':color:')).length,
        glError: gl.getError(),
        glErrors,
        clip: {
    ...
    console.log('Native Dark Elixir Drill GPU comparison', browserName, witness.category, {
