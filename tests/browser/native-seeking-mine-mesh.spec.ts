...
import witness from '../fixtures/native-seeking-mine-mesh/manifest.json' with { type: 'json' };

test.use({ viewport: { width: 1250, height: 2450 } });

test('native Seeking Air Mine clips and Info match source pixels and survive context restoration', async ({
  page,
  browserName,
}) => {
  ...
  const report = await page.evaluate(async (reference) => {
    ...
    const { default: info } = await import('/reference/seeking-mine/info.json');
    const graphs = { world, info };
    ...
    preloadNativeMeshes(scene, world, 'seeking-mine-test-world');
    preloadNativeMeshes(scene, info, 'seeking-mine-test-info');
    scene.load.image(
      'seeking-mine-source-reference',
      '/tests/fixtures/native-seeking-mine-mesh/reference.png',
    );
    ...
    const views = reference.cases.map((c) => {
      const view = new NativeSceneView(scene, `seeking-mine-test-${c.graph}`);
      const poses = nativeScenePoses(graphs[c.graph], c.export, c.time, {}, c.root);
    ...
    const image = scene.add.image(0, 0, 'seeking-mine-source-reference').setOrigin(0, 0);
    ...
    return {
      ...
      tintedTextures: scene.textures
        .getTextureKeys()
        .filter((k) => k.startsWith('seeking-mine-test') && k.includes(':color:')).length,
      glError: gl.getError(),
      glErrors,
      clip: {
  ...
  console.log('Native Seeking Air Mine GPU comparison', browserName, JSON.stringify(report));
  await page.screenshot({
    path: `output/playtest/native-seeking-mine-${browserName}.png`,
    clip: report.clip,
  });
