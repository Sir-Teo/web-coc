...
const audit = (page: Page) => page.evaluate(() => window.__cannonHandlingAudit);
...
async function setup(page: Page) {
  ...
  await page.evaluate(async () => {
    ...
    audio.unlock();
    const record = (window.__cannonHandlingAudit = { events: [], samples: [], tones: [] });
    m.onEffect = (fx) => {
      if (fx.type.startsWith('cannon-')) record.events.push(structuredClone(fx));
      scene.effect(fx);
    };
  ...
  await expect
    .poll(() =>
      page.evaluate(() =>
        ['cannon-cannon_pickup3', 'cannon-cannon_drop2'].every((key) =>
...
for (const width of [1440, 390])
  test(`native Cannon move, blocked drop and placement use one original sound at ${width}px`, async ({
    page,
    browserName,
  }) => {
    ...
    expect((await audit(page)).events.map((e) => e.type)).toEqual(['cannon-pickup']);
    ...
    expect(state.events).toEqual([
      { type: 'cannon-pickup', sourceId: 3, x: 11.5, y: 11.5 },
      { type: 'cannon-place', sourceId: 3, x: 7.5, y: 11.5 },
    ]);
    expect(state.samples.map((s) => s.sample)).toEqual([
      'cannon-cannon_pickup3',
      'cannon-cannon_drop2',
    ]);
    expect(state.samples[0].rate).toBe(1);
    expect(state.samples[0].gain).toBeCloseTo(0.096, 8);
    ...
    const grass = await page.evaluate(() => {
      const { scene } = window.__game;
      scene.paused = true;
      const event = scene.cannonPresentation.homeEffects.at(-1);
      scene.renderClock = (event.at + 0.14) * 1000;
      scene.drawOverlay();
      return [...scene.cannonPresentation.effects.values()].flatMap((v) =>
        v.objects.map((o) => ({
          x: o.x,
          y: o.y,
          depth: o.depth,
          kind: o.getData('nativeCannonEffect').key.includes(':home-place:') ? 'place' : 'pickup',
        })),
      );
    });
    ...
    await page.screenshot({
      path: `output/playtest/cannon-place-${width}-${browserName}.png`,
    });
    await page.evaluate(() => {
      const { scene } = window.__game;
    ...
    expect(await page.evaluate(() => window.__game.scene.cannonPresentation.effects.size)).toBe(0);
  });

test('home Cannon effects respect reduced motion, mute, cancellation and battle transitions', async ({
  page,
}) => {
  await setup(page);
  const result = await page.evaluate(() => {
    ...
    draw();
    const reduced = {
      grass: scene.cannonPresentation.effects.size,
    ...
    draw();
    const cancelled = {
      events: scene.cannonPresentation.homeEffects.length,
      sounds: audio.samples.active.size,
    };
    ...
    draw();
    const active = scene.cannonPresentation.effects.size;
    scene.sys.pause();
    ...
    draw();
    const battle = {
      home: scene.cannonPresentation.homeEffects.length,
      grass: scene.cannonPresentation.effects.size,
      sounds: audio.samples.active.size,
    };
    m.finishBattle();
