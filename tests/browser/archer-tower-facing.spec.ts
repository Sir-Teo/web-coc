...
test('rooftop archers track targets using original directional meshes', async ({
  page,
  browserName,
}) => {
      ...
      for (const [dx, dy, direction, flip] of directions) {
        ...
        if (
          ...
          objects.some((object) => {
            ...
            return pose.direction !== direction || pose.flip !== flip;
          })
        )
    ...
    for (const [row, level] of [1, 5, 8, 11, 13, 16, 19, 20, 21].entries()) {
      ...
      for (const [column, [dx, dy, direction, flip]] of directions.entries()) {
        ...
        const x = column * 295 + 145,
          y = row * 205 + 190;
        scene.add.text(x - 100, y - 190, `${level}: view ${direction} ${flip ? 'left' : 'right'}`, {
          fontSize: '18px',
        });
  ...
  await page.screenshot({ path: `output/playtest/archer-tower-facing-${browserName}.png` });
});
