...
import source from '../reference/dark-drill/art-source.json';
import definition from '../reference/dark-drill/native.json';
...
it('retains every original Dark Elixir Drill tier, building state and effect export', () => {
  expect(raw.exports).toEqual(definition.artInventory.exports);
  expect(Object.keys(raw.exports)).toHaveLength(39);
  expect(Object.keys(raw.clips)).toHaveLength(46);
  expect(Object.keys(raw.shapes)).toHaveLength(97);
  expect(source.definitionSha256).toBe(
    createHash('sha256').update(readFileSync('reference/dark-drill/native.json')).digest('hex'),
  );
  ...
  expect(Object.values(raw.clips).filter((c) => c.blending.includes(3))).toHaveLength(0);
