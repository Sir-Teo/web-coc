#!/usr/bin/env python3
"""Capture every original Dark Elixir Drill building/effect graph and exact source texels."""
import argparse
import hashlib
import json
...
def build():
    ...
    definition_path = ROOT / 'reference/dark-drill/native.json'
    ...
    graph = capture_graph(sc, inventory['exports'], allowed_blends=(0,))
    ...
    assets, textures, runtime = crop_textures(graph, decoded, 'assets/dark-drill-native/texture')
    ...
    return assets, {'reference/dark-drill/art-runtime.json': runtime,
                    'reference/dark-drill/art-source.json': metadata}
...
if __name__ == '__main__':
    ...
    print('Verified complete Dark Elixir Drill source artwork' if args.check else 'Wrote complete Dark Elixir Drill source artwork')
