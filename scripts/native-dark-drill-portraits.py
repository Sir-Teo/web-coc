#!/usr/bin/env python3
"""Original transparent Dark Elixir Drill portraits and source-coordinate registration."""
import argparse
import json
import numpy as np
...
def build():
    meta = json.loads((ROOT / 'reference/dark-drill/art-source.json').read_text())
    graph = meta['graph']
    textures = {int(t): np.array(decode_sctx(source(f'sc/buildings_{t}.sctx', meta['sources']))) / 255 for t in meta['textures']}
    def points(poses):
        result = []
        for pose in poses:
            require('group' not in pose, 'Unexpected isolated Drill blend group')
    ...
    for level in range(1, 12):
        name = f'darkelixir_pump_lvl{level}'
        def sample(root):
            return nodes(graph, graph['exports']['dark_elixir_pump_base'], 0, root) + nodes(graph, graph['exports'][name], 0, root)
        ...
        path = f'assets/dark-drill-native/portrait/{level}.png'
        images[path] = Image.fromarray(rgba, 'RGBA')
        rows.append(dict(level=level, path=path, width=width, height=height,
    ...
    return images, dict(sourceSha256=digest((ROOT/'reference/dark-drill/art-source.json').read_bytes()),
...
if __name__ == '__main__':
    ...
    path = ROOT / 'reference/dark-drill/portraits.json'
    data = json.dumps(metadata, indent=2) + '\n'
    if args.check:
        require(path.read_text() == data, 'Portrait registration differs')
    else:
        path.write_text(data)
    print('Verified original Dark Elixir Drill portraits' if args.check else 'Wrote original Dark Elixir Drill portraits')
