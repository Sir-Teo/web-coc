...
self.addEventListener('install',event=>{event.waitUntil((async()=>{const cache=await caches.open(CACHE);for(let i=0;i<ASSETS.length;i+=25){await Promise.all(ASSETS.slice(i,i+25).map(u=>cache.add(u).catch(()=>{})));}})());self.skipWaiting();});
