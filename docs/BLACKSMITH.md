...
The signed CSV decoding procedure is recorded in [KING-COMBAT.md](KING-COMBAT.md). Blank cells inherit within a named record. Equipment costs on a current row buy the **next** level; Blacksmith requirements describe the current row. Equipment levels 1–9 require Blacksmith 1; level 10 requires Blacksmith 3 (TH10). The home Blacksmith is deliberately held at level 1, since nothing above it is implemented. Closing that gap means equipment levels 10–18, which the source gates at Blacksmith 3, 5 and 7. Their tables are complete in the pinned sources — passive stats, Common and Rare Ore costs and ability levels 5 to 7, whose summons rise 36/40/44 and whose rage reaches +155% damage and 4.8 tiles/s — so the obstacle is not data but ore income. Those nine levels cost about 21,500 Shiny and 1,600 Glowy Ore per item, roughly 29,500 gems at the missing-ore rate, against the 40 gems a full obstacle cycle yields. See [TOWNHALL-TIERS.md](TOWNHALL-TIERS.md).
