...
`tests/townhall-9.test.ts` checks the home unlock, count gate, both paid upgrades and the stored mode. `tests/browser/xbow.spec.ts` checks the TH9 shop gate, all 26 preview keys, desktop/phone info and save reload, live aiming, physical bolt meshes, empty-state cleanup, replay rewind, home-state isolation, reduced motion and simultaneous native audio. The existing Santa replay/GPU and generic projectile timing checks remain part of integrated verification. `scripts/production-check.mjs` requires all 36 X-Bow asset requests to succeed in the built app.
