...
`art/source/bombtower-v1/` retains the two authored tower PNGs, the four-pose Bomber sheet, exposed bomb PNG and exact built-in generation prompts. `scripts/bomb-tower-assets.mjs` produces two transparent 384×512 bases, matching complete preview composites, a four-frame 256×256 Bomber atlas and a 192×192 destruction bomb. `--check` verifies byte-identical derivatives. These authored assets still supply the live presentation.
...
[The pinned native reference](../reference/bombtower/README.md) now preserves all 13 source bodies, foundations/scaffolds/rubble, three rooftop defender families with directional idle/attack clips, three projectile variants, four death bombs and six original sounds. Its importer reproduces all five texture crops and every sound byte from 19 checksum-pinned inputs. Source effects and animation-table fields are retained separately from local interpretations. Independent CPU/GPU witnesses cover 45 compositions.

These assets are prepared for integration. The live body/defender placement, directional selection, animation action frame, original impacts/explosions and audio still need integration and verification. Level 3 changes the defender, projectile and hit/destruction effects as well as HP, DPS and death damage. The campaign guard remains in place until those behaviors are supported; the playable native catalog remains its first 50 villages.
