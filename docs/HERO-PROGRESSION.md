...
- Building ceilings for TH1–8 were originally checked against [Clash Ninja's maximum-level tables](https://www.clash.ninja/guides/max-levels-for-each-th). They are now pinned against the client's own tier tables in [reference/townhall](../reference/townhall/README.md), which agree with that column and supply the Town Hall 9 tier; see [TOWNHALL-9.md](TOWNHALL-9.md). The catalog remains a subset. Building counts, housing, costs, timers, health and damage that the pinned tables do not cover are still local values.
