...
| Set Town Hall | Set TH1–9 directly and refresh its hitpoints. Existing buildings remain intact when lowering the Town Hall. |
...
| Set King level | Change the unlocked King's level within the current TH/Hero Hall cap. For level 30: unlock King, set TH9, max existing buildings, then set King level 30. |
