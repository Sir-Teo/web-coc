...
The [original Air Sweeper source foundation](../reference/air-sweeper/README.md) now preserves all seven levels, the 360-frame nozzle/sector controls, all 325 loading frames and source labels, construction/upgrade/rubble/base art, original projectile, particles and four sounds. Six original texture crops and 56 registered previews reproduce from 16 fingerprint-verified inputs. Independent source-pixel checks cover 128 cases per browser. These assets are packaged for subsequent integration; the 32 authored sprites and existing four-level combat table still drive the live game. Native loading-clock handoff and procedural shockwave rendering remain unresolved.
