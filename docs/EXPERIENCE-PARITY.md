...
Spring behavior follows the direction in Supercell’s [October 2025 update notes](https://supercell.com/en/games/clashofclans/blog/release-notes/get-ready-for-ranked-update/): one largest-housing target, ejection within capacity, damage and pushback otherwise, and prevention of simultaneous spring stacking. The local roster has no oversized ground unit yet; that branch is tested with an enlarged tank fixture. Bomb fuses, trap statistics, housing capacities, unlock/count tables, and campaign tuning are local balancing choices, not validated live-game numbers. Real asynchronous defenses and rearming between those attacks still require a server.
...
| --- | --- | --- |
| Village building and editing | Partial | Placement, wall runs, collection, builders, upgrades, undo/redo, and layout slots work. Trees and rocks now block building placement and support paid removal, cancellation, offline completion and persistent gem-cycle rewards. Trees regrow every eight hours on eligible ground with a one-tile buffer, a 45-obstacle cap and deterministic offline catch-up; saved layouts and undo/redo respect new trees. Missing the full obstacle/decorations catalog, wall-row selection/upgrades, exact placement conveniences, and the live game's full building catalog. |
...
| Defenses and traps | Partial | Cannons, archer towers, mortars, air defenses, Wizard Towers and four hidden trap types work. Missing concealed attacking defenses, remaining trap/defense catalog, complete level art, exact live statistics, and asynchronous defensive attacks. |
