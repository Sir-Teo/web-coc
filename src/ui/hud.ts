 ...
 <div class="edit-toolbar">
  ...
  private battleHUD() {
    ...
    const limitedStorage =
      !b.practice &&
      !m.replay &&
      (['gold', 'elixir'] as const).some((k) => (b.lootRoom?.[k] ?? v[k]) < v[k]);
    const lootBar = (k: 'gold' | 'elixir') =>
      `<div class="loot-row">${k === 'gold' ? coin : elixir}<div class="loot-track"><i data-lootbar="${k}" style="width:${pct((lootLeft(k) / Math.max(1, v[k])) * 100)}"></i></div><b data-loot="${k}">${n(lootLeft(k))}</b></div>`;
    return `<div class="battle-enemy"><span class="eyebrow">${m.replay ? (m.replay.recordId === null ? 'SHARED REPLAY' : 'ATTACK REPLAY') : b.practice ? 'PRACTICE ATTACK' : 'ENEMY VILLAGE'}</span><h2>${v.name}</h2>${m.replay ? '<small class="practice-note">Recorded attack · Watch &amp; learn</small>' : b.practice ? '<small class="practice-note">Your village and army are safe.<br>No loot or trophies at stake.</small>' : `<small>AVAILABLE LOOT</small><div class="loot-bars">${lootBar('gold')}${lootBar('elixir')}</div>${limitedStorage ? '<small class="loot-capacity-note">Loot beyond your storage capacity will be lost.</small>' : ''}`}</div>
