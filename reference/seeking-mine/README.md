...
`native.json` retains original sparse rows, full source graphs, texture sampling regions, source hashes and fidelity limits. `runtime.json` and `info.json` retain original geometry and transforms with UVs remapped to lossless crops. `combat.json` and `effects.json` expose focused source projections, including all four projectile records’ export, shadow, speed, height, offset, scale, play-once flag and trail emitter. Six cropped textures, four world previews, the original Info picture and five Ogg files total **16 assets / 690,354 bytes**.
