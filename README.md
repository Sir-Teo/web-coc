...
- Attack opens the native 90-village campaign catalog. The first 54 layouts are supported, including all 59 level-three Seeking Air Mines in villages 52–54, Invaders' original level-3 Bomb Tower and Goblin Picnic's native Santa Trap animation and five timed strikes; later missing mechanics are labeled Coming soon. Campaign scouting and combat have **no time limit**, award no trophies and draw from a finite saved treasury. Practice retains 30-second scouting and a three-minute attack clock. See [Santa Trap details and fidelity limits](docs/SANTA-TRAP.md).
...
- Air Sweepers rotate through eight directions and push flying attackers with traveling gusts. Seeking Air Mines use eight source damage levels, four original animated families, original particles/sounds and the original Info portrait. Home mines deal 1,500 damage; campaign mines retain their source levels. Layouts and replays preserve both defenses. See [air-control behavior and fidelity limits](docs/AIR-CONTROL.md).
...
- Native campaign names, loot, layout coordinates, levels, hitpoints and scenery come from a pinned public client bundle. The first 58 supported layouts preserve all their entities. Legacy twelve-village progress remains separate; native Pumpkin Bombs now open Rat Valley and three other villages; Obsidian Tower includes its five level-three Skeleton Traps, and Goblin Picnic includes its Santa Trap. See [campaign coverage and fidelity limits](docs/CAMPAIGN-RULES.md).
...
The experience inventory and next implementation priorities are tracked in [docs/EXPERIENCE-PARITY.md](docs/EXPERIENCE-PARITY.md). Additional heroes, equipment, the full content roster, online defense/matchmaking, and clan systems are still missing; replays cover local attacks only, with portable file sharing and seeking, but no hosted sharing links, defense history, or playback beyond the explicitly supported combat versions 34–38. See [docs/REPLAYS.md](docs/REPLAYS.md).
