...
- Attack opens the native 90-village campaign catalog. Forty-eight layouts are supported; missing seasonal and later mechanics are labeled Coming soon. Campaign scouting and combat have **no time limit**, award no trophies and draw from a finite saved treasury. Practice retains 30-second scouting and a three-minute attack clock.
...
- Native campaign names, loot, layout coordinates, levels, hitpoints and scenery come from a pinned public client bundle. Forty-eight supported layouts preserve all their entities. Legacy twelve-village progress remains separate; native Pumpkin Bombs now open Rat Valley and three other villages; Obsidian Tower still requires level-three Skeleton Traps. See [campaign coverage and fidelity limits](docs/CAMPAIGN-RULES.md).
